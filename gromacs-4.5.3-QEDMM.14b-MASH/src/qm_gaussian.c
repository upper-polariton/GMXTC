...
printM_complex(ndim,wmatrix);
  ...
  /* I don't quite understand why, but this order i.e. Vt.wmatrix.V gives
   * results that are in agreement with Mathematica...
   * Apparently diag (zheev) returns the eigenvectors as rows, not
   * columns. So tranposing is required, but what about the CC?
   */
  dagger(ndim,Vt,V);
  M_complextimesM_complex(ndim,wmatrix,V,temp);
  M_complextimesM_complex(ndim,Vt,temp,expA);
...
/* decoherence correction by Grunucci et al (J. Chem. Phys. 126, 134114 (2007) */
  int 
    state;
  double 
...
int compute_hopping_probability(int step,t_QMrec *qm, dplx *c, dplx *U, int ndim){
  ...
  fprintf(stderr," population that leaves diabatic state %d: %lf\n",
	  current,(conj(cold[current])*cold[current]-conj(c[current])*c[current]));
  fprintf(stderr, "probability to leave diabatic state %d is %lf\n",current,
			    ...
			    rvec tdmXMM[], rvec tdmYMM[], rvec tdmZMM[],double *energies)
{
  ...
  if (fr->qr->SHmethod != eSHmethodGranucci && fr->qr->SHmethod != eSHmethodEhrenfest){
    gmx_fatal(FARGS, "Running in diabatic basis only possible for Ehrenfest of Surface hopping with local diabatization\n");
  }
  ...
  if(dodia){
	...
	ham[i]=matrix[i]+qm->matrix[i];
     ...
     for(i=nmol;i<ndim;i++){
       /* hbar*decay rate 6.582119569*10^(\[Minus]16)/10^(-12) 
	*/
  ...
  if(fr->qr->SHmethod == eSHmethodGranucci){
	...
	ener += creal(cicj*matrix[i*ndim+j])+cimag(cicj*matrix[i*ndim+j]);
	if(dodia){
	  fprintf(stderr,"ener = %12.8lf + %12.8lf\n",creal(ener),cimag(ener));
	}
      }
    }
    qm->groundstate=1-totpop;
    ener+=(qm->groundstate)*(energies[ndim-1]-cavity_dispersion(qm->n_max,qm));
    if (dodia){
  ...
  if(fr->qr->SHmethod == eSHmethodGranucci){
  ...
  else{ /* Ehrenfest, mean field gradient */
    csq = conj(qm->creal[m]+IMAG*qm->cimag[m])*(qm->creal[m]+IMAG*qm->cimag[m]);
    for(i=0;i<qm->nrQMatoms;i++){
      for(j=0;j<DIM;j++){
	fij = csq*QMgrad_S1[i][j]+(1-csq)*QMgrad_S0[i][j];
	for(p=nmol;p<ndim;p++){
    ...
    for(i=0;i<mm->nrMMatoms;i++){
      for(j=0;j<DIM;j++){
	fij = csq*MMgrad_S1[i][j]+(1-csq)*MMgrad_S0[i][j];
	for(p=nmol;p<ndim;p++){
	  cmcp=conj(qm->creal[m]+IMAG*qm->cimag[m])*(qm->creal[p]+IMAG*qm->cimag[p]);
	  cmcp*=sqrt(cavity_dispersion(p-nmol,qm)/V0_2EP)*cexp(-IMAG*2*M_PI*(p-nmol)/L_au*m*L_au/((double) nmol));
		   ...
		   rvec tdmXMM[], rvec tdmYMM[], rvec tdmZMM[],double *energies)
{
  double
    ...
    E0_norm_sq,V0_2EP,u[3],QMener=0.;
  dplx
    fij,csq,cmcp,*ham,
...
//  
  if((dodia) ){
    ...
    for(i=0;i<ndim;i++){
      fprintf(evout,
	      "step %4d Eigenvector %4d gap %12.8lf (c*c: %12.8lf + %12.8lf I):",
	      step,i,eigval[i]-(qm->groundstate,
  ...
  if(fr->qr->SHmethod != eSHmethodEhrenfest){
	...
	a_sump += eigvec[p*ndim+nmol+i]*sqrt(cavity_dispersion(i,qm)/V0_2EP)*cexp(-IMAG*2*M_PI*i/L_au*m*L_au/((double) nmol));
      ...
      for(i=0;i<qm->nrQMatoms;i++){
//        fprintf(stderr,"molecule %d, atom %d: fij = ",m,i);
	for(j=0;j<DIM;j++){
	  /* diagonal term */
	  fij =(betasq*QMgrad_S1[i][j]+(1-betasq)*QMgrad_S0[i][j]);
...
//          fprintf(stderr, "%lf + %lf I ",creal(fij),cimag(fij)); 
	  f[i][j]      += creal(fij);
	  fshift[i][j] += creal(fij);
        }
//        fprintf(stderr,"\n");
...
//    couplings[m*((qm->n_max)+1)+i] = -iprod(tdm,u)*sqrt(cavity_dispersion(i,qm)/V0_2EP)*cexp(IMAG*2*M_PI*i/L_au*m*L_au/((double) nmol));
  ...
  for (k=0;k<nmol;k++){
    for (j=0;j<((qm->n_max)+1);j++){
      /* GG @ 5.1: altered which block we take the complex conjugate
